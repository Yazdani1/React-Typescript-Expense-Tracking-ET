...
import { createJobWishList, deleteJobWishList, CreateJobWishListProps, getSingleJobWishlist } from '../../../services/API';
...
const JobPostsPublicCard: FC<JobPostsPublicCardProps> = ({ jobpost }) => {
  ...
  const isJobPostSaved = allJobWishList && allJobWishList.some((job) => job.jobPostId?._id === jobpost?._id);
  ...
  return (
    <div className={style.job_post_container}>
      <Link to={'/job-details/' + jobpost.slug} style={{ textDecoration: 'none', color: 'inherit' }}>
        <h6>{jobpost.title}</h6>
      ...
      <div className={style.job_post_bottom_row}>
        ...
        {userProfileDetails && (
          <>
            {isJobPostSaved && (
              <p onClick={handleDeleteJobWishList} style={{ cursor: 'pointer' }}>
                <AiFillHeart size={25} />
                Saved
              </p>
            )}
