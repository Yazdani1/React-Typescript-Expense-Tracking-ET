...
const JobApplicationCard: FC<JobApplicationCardProps> = ({ job_application }) => {
  // Context API

  const { allJobApplication, loadJobApplication, loading } = useJobApplicationContext();
  ...
  return (
    <div className={style.job_application_card_container}>
      <div style={{ flex: 2 }}>
        <Link to={'/job-details/' + job_application.jobPostId?.slug} style={{ textDecoration: 'none', color: 'inherit' }}>
      ...
      <DropDownList showJobWithdraw={true} withdrawJobApplication={onOpenModal} />

      {/* Modal box to to delete job application */}

      <ConfirmModal open={open} onCloseModal={onCloseModal} deletePost={withdrawJobApplication} title="Withdraw job application" showDeleteButton>
